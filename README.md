...
> 오늘 사용한 크래커를 자동 합산하고, 설정한 일일 목표의 멈춤 구간에 도달하면 메시지 전송과 재생성을 차단합니다.

### 기본 사용법

1. 채팅 입력창 위의 **크래커 가드** 상태 행을 누릅니다.
2. 하루에 맞추고 싶은 **일일 목표**를 입력하거나 `−` / `+`로 100개씩 조절합니다.
3. 불규칙한 크래커 소모량을 고려한 **허용 오차(±)**를 입력하거나 `−` / `+`로 50개씩 조절합니다.
4. 필요하면 **재생성도 막기**를 켭니다.
5. 바꾼 설정은 저장 버튼 없이 바로 적용되고, 현재 사용량을 기준으로 자동 감시합니다.

> **예시:** 일일 목표가 `1,000개`, 허용 오차가 `±200개`라면 멈춤 구간은 `800~1,200개`입니다. 최근 1회 소모량을 참고하여 목표에 더 가까운 지점에서 멈춥니다.
...
| **멈춤 구간** | 일일 목표와 허용 오차로 멈춤 구간을 계산하고, 설정창 게이지에 구간과 목표 지점을 표시 |
| **전송 차단** | 전송 버튼과 Enter 키를 통한 메시지 전송 차단 |
| **재생성 차단** | 리롤·다시 생성으로 추가 소모되는 크래커 차단 |
| **자동 갱신** | 20초마다, 창 복귀 시, 메시지 전송 후 사용량 갱신. 설정창의 **지금 확인**으로 바로 갱신 |
| **라디오존데 호환** | 라디오존데 사용 시 `가드 → 라디오존데 → 입력창` 순서로 표시 |
| **테마 호환** | 크랙의 라이트·다크 모드와 입력창에 적용된 테마를 상태 행과 설정창에 반영 |
